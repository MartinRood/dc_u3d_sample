Shader "YuLongZhi/SpecularSimulateTerrain"
{
	Properties
	{
		...
		_ShaderLightDir ("Shader Light Dir", Vector) = (0, -1, 0, 0)
	}
	...
	SubShader
	{
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed3 lay2 = tex2D(_Splat2, uv_Splat2);
				...
				c.a = 1.0;
				c.rgb = (lay0 * splat_control.r + lay1 * splat_control.g + lay2 * splat_control.b + lay0 * splat_control.a);
				...
				fixed3 _Spec;
				_Spec = (_Spec0 * splat_control.r + _Spec1 * splat_control.g + _Spec2 * splat_control.b + _Spec0 * splat_control.a);
				...
				half3 viewDir = normalize(UnityWorldSpaceViewDir(i.posWorld) - half3(0, 9, 0));
				...
				half2 rlPow4AndFresnelTerm = Pow4(half2(dot(reflDir, normalize(_ShaderLightDir)), 1 - nv));
