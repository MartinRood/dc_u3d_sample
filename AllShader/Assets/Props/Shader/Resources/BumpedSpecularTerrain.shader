Shader "YuLongZhi/BumpedSpecularTerrain"
{
	...
	SubShader
	{
		Pass
		{
			...
			#pragma multi_compile __ RAIN_ENABLE SNOW_ENABLE
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed4 lay0 = tex2Dbias(_Splat0, half4(uv_Splat0, 0, mip_bias));
				fixed4 lay1 = tex2Dbias(_Splat1, half4(uv_Splat1, 0, mip_bias));
				fixed4 lay2 = tex2Dbias(_Splat2, half4(uv_Splat2, 0, mip_bias));
				fixed3 n0 = UnpackNormal(tex2Dbias(_Normal0, half4(uv_Splat0, 0, mip_bias)));
				fixed3 n1 = UnpackNormal(tex2Dbias(_Normal1, half4(uv_Splat1, 0, mip_bias)));
				fixed3 n2 = UnpackNormal(tex2Dbias(_Normal2, half4(uv_Splat2, 0, mip_bias)));
