    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
				...
				float4 color_0 = tex2D(_WaterTex, TRANSFORM_TEX(st_0, _WaterTex));

				float2 st_1 = (i.uv0 + time * float2(-0.01, 0.05));
				float4 color_1 = tex2D(_WaterTex, TRANSFORM_TEX(st_1, _WaterTex));

				float2 st_2 = i.uv0 + time * float2(0.03, 0.03) + color_0.rg * color_1.rg * _Distort;
				float4 color_2 = tex2D(_WaterTex, TRANSFORM_TEX(st_2, _WaterTex));

				float3 color_3 = _Color.rgb * color_2.rgb * _Intensity;

				fixed4 c = tex2D(_MainTex, i.uv0) * fixed4(color_3, 1);
